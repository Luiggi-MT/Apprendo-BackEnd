...
def get_students():
    ...
    try:
        students = db.fetch_query(
            f"""SELECT foto, username, tipoContraseña, accesibilidad, preferenciasVisualizacion, asistenteVoz, id, pista
                FROM estudiantes 
                LIMIT %s OFFSET %s""",
            (limit, offset)
        )
        ...
        for student in students:
            accesibilidad = student['accesibilidad'].split(
                ',') if student['accesibilidad'] else []
            student_list.append({
                'foto': student['foto'],
...
def delete_student(username):
    ...
    try:
        student = db.fetch_query(
            f"SELECT id FROM estudiantes WHERE username = %s", (username, ), fetchone=True)
    except Exception as e:
    ...
    try:
        db.execute_query(
            f"DELETE FROM estudiantes WHERE id = %s", (student['id'], ))
    except Exception as e:
        print(f"Error deleting student: {e}")
        return {"Error": str(e)}, 500
    try:
        deleted = db.fetch_query(
            f"SELECT id FROM estudiantes WHERE id = %s", (student['id'], ), fetchone=True)
    except Exception as e:
        print(f"Error verifying deletion: {e}")
        return {"Error": str(e)}, 500

    if deleted:
        print("Error: Student not deleted")

        return {"Error": "No se ha podido eliminar al estudiante"}, 400

    student_dir = os.path.join(
        UPLOAD_FOLDER, 'estudiantes', str(student['id']))
...
def get_student(username):
    ...
    try:
        ...
        count_result = db.fetch_query(
            "SELECT COUNT(*) FROM estudiantes WHERE username LIKE %s", (f"%{username}%",), fetchone=True)
...
def create_student():
    ...
    fields = ['username', 'tipoContraseña',
              'preferenciasVisualizacion', 'asistenteVoz']
    params = [
        data.get('username'),
    ...
    if data.get('contraseña'):
        ...
        hashed_contraseña = generate_password_hash(
            contraseña, method='pbkdf2:sha256', salt_length=16)
        params.append(hashed_contraseña)
        fields.append('contraseña')

    if data.get('accesibilidad'):
        accesibilidad = data.get('accesibilidad')
        accesibilidad_str = ','.join([a.strip() for a in accesibilidad]) if isinstance(
            accesibilidad, list) else accesibilidad.strip()
...
def upload_student_photo(id):
    ...
    try:
        foto_url = db.fetch_query(
            "SELECT foto FROM estudiantes WHERE id = %s", (id,), fetchone=True)
    except Exception as e:
        return {'error': str(e)}, 500
...
def update_student(id):
    ...
    if data.get('contraseña'):
        ...
        params.append(generate_password_hash(
            data.get('contraseña'), method='pbkdf2:sha256', salt_length=16))
    ...
    if data.get('accesibilidad'):
        ...
        if isinstance(accesibilidad, list):
            accesibilidad_str = ','.join(
                [a.strip() for a in accesibilidad if a])
        else:
...
def clear_image_password(id):
    ...
    student_dir = os.path.join(
        UPLOAD_FOLDER, 'estudiantes', str(id), 'contraseñaImagen')
    if os.path.exists(student_dir):
        shutil.rmtree(student_dir)
    ...
    try:
        ...
        deleted = db.fetch_query(
            "SELECT id FROM contraseña_imagenes_estudiante WHERE id_estudiante = %s", (id,), fetchone=True)
        if deleted:
...
def imagen_password(id):
    ...
    inserted = db.fetch_query(
        "SELECT id FROM contraseña_imagenes_estudiante WHERE id_estudiante = %s AND url_imagen = %s", (id, db_path), fetchone=True)
    if not inserted:
